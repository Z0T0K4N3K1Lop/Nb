...
import requests, tempfile, shutil, json
...
class LINE:
  ...
  def clonePicture(self ,mid):
    contact = self.getContact(mid)
    profile = self.getProfile()
    profile.pictureStatus = contact.pictureStatus
    return self.updateDisplayPicture(profile.pictureStatus)
  ...
  def post_content(self, urls, data=None, files=None):
	return self.Talk._session.post(urls, headers=self._headers, data=data, files=files)
